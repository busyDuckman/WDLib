...
namespace WD_toolbox.Data.DataStructures
{
    ...
    /// </summary>
    internal struct DSTNode<elementType, resultType> : IDictionary<IList<elementType>, resultType>, IList<resultType>, ISerializable
...
where elementType : IComparable<elementType>
    {
        ...
        private enum DSTOperation { find, replace, remove };


        /// <summary>
        /// Performs the DST operation based on a key search.
        /// 
        /// Pecause the search is the part that often incurs bugs we are only using one search to perform
        /// multiple operations.
        /// </summary>
        ...
        private bool performDSTOperation(IList<elementType> key, out resultType value, resultType newValue, int keyPos, DSTOperation operation)
        {
        ...
        {
            return new InvalidOperationException(string.Format("The DST did not contain an item at position {0}.", index));
        }
