...
namespace WD_toolbox//.DotNetExtension
{
    public static class IListExtension
    {
        ...
        //NB: this used to be IReadonlyList, I think there was a reason why
        public static T GetRandomItem<T>(this IList<T> list, Random rnd)
        {
            if (list == null)
            {
        ...
        public static void AlterRandomItem<T>(this IList<T> list, Func<T, T> func, Random rnd)
        {
            if ((list != null) && (list.Count > 0))
            {
                int pos = rnd.Next(0, list.Count);
                list[pos] = func(list[pos]);
            }
        }
